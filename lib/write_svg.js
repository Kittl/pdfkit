...
export default function (doc, svg, x, y, options) {
  ...
  var SvgElemContainer = function (obj, inherits) {
    ...
    this.drawContent = function (isClip, isMask, blendMode) {
      ...
      let clipped = this.clip(),
        ...
        group;
      if (
        (this.get('opacity') < 1 || clipped || masked || blendMode) &&
        !isClip
      ) {
    ...
    this.drawInDocument = function (isClip, isMask) {
      ...
      if (styles['mix-blend-mode']) {
        this.drawContent(
          isClip,
          isMask,
          parseCSSBlendMode(styles['mix-blend-mode']),
        );
      } else {
        this.drawContent(isClip, isMask);
      }
  ...
  var SVGElemImage = function (obj, inherits) {
    SvgElemStylable.call(this, obj, inherits);
    const { src: imageSrc, properties } = imageCallback(
    ...
    try {
      ...
      warningCallback(
        'SVGElemImage: failed to open image "' + e + '" in PDFKit',
        e,
      );
    }
    if (image) {
