...
// This directly taken from SVG-to-PDF
// Contains some minor patches and changes
export default function (doc, svg, x, y, options) {
  'use strict';
  ...
  function docEndGroup(group) {
    ...
    if (doc._activeColorProfile) {
      ...
      group.resources.data.ColorSpace[doc._activeColorProfile.label] = doc._activeColorProfile.ref;
    }
    group.resources.end();
    group.xobj.end();
  ...
  function docApplyAlphaMask(group) {
    ...
    const gstate = doc.ref({
      ...
      BM: 'Normal',
      SMask: {
        // We only support alpha mask for now
        S: 'Alpha',
        G: group.xobj,
      },
    });
  ...
  function opacityToColor(color, opacity) {
  ...
  var SvgElemStylable = function (obj, inherits) {
    ...
    this.getFill = function (isClip, isMask) {
      ...
      if (fill !== 'none' && opacity && fillOpacity) {
        ...
        return opacityToColor(fill, fillOpacity * opacity);
      }
    };
    this.getStroke = function (isClip, isMask) {
      ...
      if (stroke !== 'none' && opacity && strokeOpacity) {
        ...
        return opacityToColor(stroke, strokeOpacity * opacity);
      }
    };
  };
  ...
  var SVGElemImage = function (obj, inherits) {
    SvgElemStylable.call(this, obj, inherits);
    const { src: imageSrc, properties } = imageCallback(this.attr('href') || this.attr('xlink:href') || '');
    let x = this.getLength('x', this.getVWidth(), 0),
      y = this.getLength('y', this.getVHeight(), 0),
  ...
  var SvgElemGradient = function (obj, inherits, fallback) {
    ...
    this.getPaint = function (bBox, gOpacity /* isClip, isMask */) {
      ...
      if (children.length === 1) {
        ...
        return opacityToColor(stopColor, child.get('stop-opacity') * gOpacity);
      }
      ...
      if (matrix && (matrix = validateMatrix(matrix))) {
        ...
        for (let n = 0; n < nTotal; n++) {
          ...
          for (let i = 0; i < children.length; i++) {
            ...
            stopColor = opacityToColor(stopColor, child.get('stop-opacity') * gOpacity);
  ...
  if (typeof fontCallback !== 'function') {
    fontCallback = function (family, bold, italic, fontOptions) {
      ...
      } /*if (family.match(/(?:^|,)\s*sans-serif\s*$/) || true)*/ else {
        if (bold && italic) {
